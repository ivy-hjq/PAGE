...
def train_FedAvg(data_obj, act_prob ,learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, device, rand_seed=0):
    ...
    weight_list = np.asarray([len(data_obj.clnt_train_loader[i])*batch_size for i in range(n_clnt)])
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                ###
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.clnt_test_loader[clnt], clnt_models[clnt], data_obj.dataset, device)
                tst_perf_global[clnt].append([loss_tst, acc1_tst, acc5_tst])
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt] = train_model_(clnt_models[clnt], data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, data_obj.dataset, device)
                
                #test_perf_clnt after train using local test
                ###
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.clnt_test_loader[clnt], clnt_models[clnt], data_obj.dataset, device)
                tst_perf_clnt[clnt].append([loss_tst, acc1_tst, acc5_tst])
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
            ...
            loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.test_loader, all_model, data_obj.dataset, device)
            tst_perf_all.append([loss_tst, acc1_tst, acc5_tst])
            print("**** Communication all %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, acc1_tst, acc5_tst, loss_tst))
            ...
            if ((i+1) % save_period == 0):
                ...
                np.save('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, (i+1)), tst_perf_all)
...
def train_SCAFFOLD(data_obj, act_prob, learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, device, rand_seed=0, global_learning_rate=1):
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt] = train_scaffold_mdl_(clnt_models[clnt], model_func, state_params_diff_curr, data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, data_obj.dataset, device)
                #test_perf_clnt after train using local test
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.clnt_test_loader[clnt], clnt_models[clnt], data_obj.dataset, device)
                tst_perf_clnt[clnt].append([loss_tst, acc1_tst, acc5_tst])
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                n_minibatch = epoch*len(data_obj.clnt_train_loader[clnt])
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                #test_perf_global berfore train using local test
                ###
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.clnt_test_loader[clnt], model, data_obj.dataset, device)
                tst_perf_global[clnt].append([loss_tst, acc1_tst, acc5_tst])
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                # Scale down
                ...
                clnt_models[clnt] = train_feddyn_mdl_(model, model_func, alpha_coef_adpt, cld_mdl_param_tensor, local_param_list_curr, data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, data_obj.dataset, device)
            ...
            loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.test_loader, cld_model, data_obj.dataset, device)
            tst_perf_all.append([loss_tst, acc1_tst, acc5_tst])
            print("**** Communication all %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, acc1_tst, acc5_tst, loss_tst))
            print("**** Communication all %3d, Cent Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, acc1_tst, acc5_tst, loss_tst))
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt]= train_fedprox_mdl_(clnt_models[clnt], avg_model_param_tensor, mu, data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, data_obj.dataset, device)
            ...
            loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.test_loader, avg_model, data_obj.dataset, device)
            tst_perf_all.append([loss_tst, acc1_tst, acc5_tst])
            print("**** Communication all %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, acc1_tst, acc5_tst, loss_tst))
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt],local_params[clnt], personalized_params[clnt] = train_pfedme_mdl_(clnt_models[clnt], local_params[clnt], personalized_params[clnt], data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, lamda, K, mu, data_obj.dataset, device)
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt], clnt_models_per[clnt] = train_ditto_mdl_(clnt_models[clnt], clnt_models_per[clnt], data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, plocal_steps, mu, data_obj.dataset, device)
               
                #test_perf_clnt after train using local test
                ###
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.clnt_test_loader[clnt], clnt_models_per[clnt], data_obj.dataset, device)
                tst_perf_clnt[clnt].append([loss_tst, acc1_tst, acc5_tst])
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt] = train_ala_mdl_(clnt_models[clnt], copy.deepcopy(all_model),data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, eta, rand_percent, layer_idx,  clnt_alas, clnt, data_obj.dataset, device)
...
def train_fedrod(data_obj, act_prob ,learning_rate, batch_size, epoch, com_amount, print_per, num_classes, model_func, init_model, save_period, lr_decay_per_round, device, rand_seed=0):
    ...
    all_model = model_func().to(device)
    all_head = copy.deepcopy(all_model.model.fc)
    all_model.model.fc = nn.Identity()
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt] = model_func().to(device)
                clnt_head[clnt] = copy.deepcopy(clnt_models[clnt].model.fc)
                clnt_models[clnt].model.fc = nn.Identity()
                ...
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt],clnt_head_per[clnt] = train_fedrod_mdl_(clnt_models[clnt], clnt_head_per[clnt],data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, num_classes, data_obj.dataset, device)
                #test_perf_clnt after train using local test
                ###
                
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_fedrod_(data_obj.clnt_test_loader[clnt], clnt_models[clnt], clnt_head_per[clnt], data_obj.dataset, device)
                tst_perf_clnt[clnt].append([loss_tst, acc1_tst, acc5_tst])
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
            ...
            # Scale with weights
            mf=model_func()
            mf_head = copy.deepcopy(mf.model.fc)
            mf.model.fc = nn.Identity()
...
def train_fedrecon(data_obj, act_prob ,learning_rate, batch_size, recon_epochs, pers_epochs, recon_lr, pers_lr, s_lr, n_test_client, supset_ratio, com_amount, print_per, model_func, init_model, save_period, lr_decay_per_round, device, rand_seed=0):
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                print("**** Communication sel %3d, client %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, clnt, acc1_tst, acc5_tst, loss_tst))
                ### train
                clnt_models[clnt] = train_fedrecon_mdl_(clnt_models[clnt],data_obj.clnt_train_loader[clnt], learning_rate * (lr_decay_per_round ** i), batch_size, recon_epochs, pers_epochs, recon_lr, pers_lr, supset_ratio, print_per,  data_obj.dataset, device)
                #test_perf_clnt after train using local test
                ###
                loss_tst, acc1_tst, acc5_tst = get_acc_loss_(data_obj.clnt_test_loader[clnt], clnt_models[clnt], data_obj.dataset, device)
                tst_perf_clnt[clnt].append([loss_tst, acc1_tst, acc5_tst])
                ...
                # clnt_params_list[clnt] = get_mdl_params([clnt_models[clnt].base], n_par)[0]
            ...
            loss_tst, acc1_tst, acc5_tst = get_acc_loss_fedrecon_(data_obj.test_clnt_recon_loader, data_obj.test_clnt_test_loader,all_model, data_obj.dataset, device,n_test_client, recon_epochs, supset_ratio ,recon_lr, batch_size)
            tst_perf_all.append([loss_tst, acc1_tst, acc5_tst])
            print("**** Communication all %3d, Test Acc1: %.4f Acc5: %.4f , Loss: %.4f" %(i+1, acc1_tst, acc5_tst, loss_tst))
