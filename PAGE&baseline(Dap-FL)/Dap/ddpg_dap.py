...
class DDPG(object):
    def __init__(self, nb_states, nb_actions, nb_agents,args,hidden1,hidden2,bsize,lr_max,lr_min,epoch_max):
        ...
        for i in range (self.nb_agents):
            self.actor_optim_list.append(Adam(self.actor_list[i].parameters(), lr=args.prate))
            self.critic_optim_list.append(Adam(self.critic_list[i].parameters(), lr=args.rate))
    ...
    def random_a(self):
        np.random.seed(None)
        action = np.random.uniform(self.lr_max,self.lr_min,(self.nb_agents,self.nb_actions))#挑选nb_agents*nb_actions个范围在[1e-4,1e-2]的action(lr)
        for i in range(self.nb_agents):
    ...
    def random_sa(self):
        np.random.seed(None)
        action = np.ones((self.nb_agents,self.nb_actions),dtype=np.float64)
        for i in range(self.nb_agents):
