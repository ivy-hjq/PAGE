...
class DatasetSynthetic:
    def __init__(self, alpha, beta, theta, iid_sol, iid_data, n_dim, n_clnt, n_cls, avg_data, split_ratio, split_ratio_global, name_prefix,data_path):
        self.dataset = 'synt'
...
def partition_data(datadir, partition, n_nets, alpha,unbalanced_sgm):
        ...
        if diff!=0:
            for cid in range(n_nets):
                if client_sample_nums[cid] > diff:
                    ...
                    break
        # client_sample_nums=np.cumsum(client_sample_nums).astype(int)
...
def get_local_dataloader(datadir, train_bs, test_bs, dataidxs=None, ratio=0.8):
    if 'cifar' in datadir:
        train_transform, test_transform = _data_transforms_cifar(datadir)
        dl_obj = CIFAR_truncated
        workers=0
    ...
    train_ds = dl_obj(datadir, dataidxs=train_dataidxs, train=True, transform=train_transform, download=True)
    test_ds = dl_obj(datadir, dataidxs=test_dataidxs, train=True, transform=test_transform, download=True)
...
def load_partition_data(data_dir, partition_method, partition_alpha, client_number, batch_size,ratio):
    ...
    for client_idx in range(client_number):
        ...
        train_data_local, test_data_local ,train_dataset= get_local_dataloader(data_dir, batch_size, batch_size, dataidxs,ratio)
...
class ImagenetObjectCrop_noniid:
    def __init__(self, dataset, data_path, n_client, rule='hetero', unbalanced_sgm=0, rule_arg=0.3,split_ratio=0.8,batch_size=32):
        self.n_client = n_client
        self.dataset = dataset
        self.rule = rule
        self.name = "%s_%d_%s_%s" %(self.dataset, self.n_client, self.rule, rule_arg)
        self.train_loader,self.test_loader,self.clnt_train_loader,self.clnt_test_loader=load_partition_data(data_dir=data_path,partition_method=rule,partition_alpha=rule_arg,unbalanced_sgm=unbalanced_sgm,client_number=n_client,batch_size=batch_size,ratio=split_ratio)
